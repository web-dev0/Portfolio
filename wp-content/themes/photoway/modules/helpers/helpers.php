...
if( !function_exists( 'photoway_enqueue' ) ):
/**
...
* @return void
*/
function photoway_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'photoway_scripts' , $scripts );
...
if( ! function_exists( 'photoway_get_related_posts' ) ):
/**
* Get related posts
*
* @since Photoway 1.0.0
...
* 
*/
function photoway_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function photoway_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'photoway_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'photoway_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function photoway_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'photoway' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'photoway_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function photoway_get_youtube_id( $url ) {
...
if( ! function_exists( 'photoway_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Photoway 1.0.0
* @uses photoway_sort_category()
* @return array
*/
function photoway_get_the_category( $id = false ){
    $failed = true;
    ...
    if ( class_exists( 'WPSEO_Primary_Term' ) ){
        ...
        $photoway_cat[0] = get_term( $wpseo_primary_term );

        if ( !is_wp_error( $photoway_cat[0] ) ) { 
           $failed = false;
        }
    }
    ...
    if( $failed ){
      ...
      $photoway_cat = get_the_category( $id );
      usort( $photoway_cat, 'photoway_sort_category' );  
    }
...
if( ! function_exists( 'photoway_sort_category' ) ):
/**
* Helper function for photoway_get_the_category()
*
* @since Photoway 1.0.0
*/
function photoway_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'photoway_explode_string_to_int' ) ):
/**
...
*/
function photoway_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'photoway_get_menu' ) ):
/**
* Returns wp nav
*
* @since Photoway 1.0.0
...
* @return String
*/
function photoway_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'photoway_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'photoway_'.$location.'_menu', $args ) );
        break;

        case 'secondary':
            $args[ 'fallback_cb' ] = 'photoway_secondary_menu_fb';
        ...
        case 'footer':
            $args[ 'fallback_cb' ] = 'photoway_footer_menu_fb';
...
if( !function_exists( 'photoway_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Photoway 1.0.0
*/
function photoway_hex2rgba($color, $opacity = false) {
