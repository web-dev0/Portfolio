...
if( ! class_exists( 'Photoway_Customizer' ) ):

	class Photoway_Customizer{
		...
		* 
		* @since  Photoway 1.0.0
		* @uses   filter "Photoway_Customizer_defaults"
		* @access public
		* @return array
		*/
	    public function get_defaults(){

	    	return apply_filters( 'Photoway_Customizer_Defaults', array() );
	    }
		...
		* @since  Photoway 1.0.0
		* @uses   get_defaults()
		* @uses   filter "Photoway_Customizer_Fields"
		* @access public
		* @param  string $id
		* @return string | false
		*/
		public function get( $id ){
			...
			$fields   = apply_filters( 'Photoway_Customizer_Fields', array() );
		...
		*/
		public function register( $wp_customize ){
			...
			do_action( 'photoway_customize_register', $wp_customize );

			$this->customize = $wp_customize;

			$this->fields   = apply_filters( 'Photoway_Customizer_Fields', array() );
			$this->panels   = apply_filters( 'Photoway_Customizer_Panels', $this->panels );
			$this->sections = apply_filters( 'Photoway_Customizer_Sections', $this->sections );

			$this->defaults = apply_filters( 'Photoway_Customizer_Defaults', $this->defaults );
			...
			foreach( $this->fields as $key => $field ){
				...
				switch ( $field[ 'type' ] ) {
					...
					case 'number':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'photoway_sanitize_number';
						}
					...
					case 'checkbox':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'photoway_sanitize_checkbox';
						}
					...
					case 'radio':
						...
						if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
							$field[ 'sanitize_callback' ] = 'photoway_sanitize_choice';
						}
					...
					case 'dropdown-pages':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'photoway_sanitize_dropdown_pages';
					}
					break;
					
					case 'dropdown-posts':
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'photoway_sanitize_dropdown_posts';
					}
					break;
					...
					if( ! isset( $field[ 'sanitize_callback' ] ) || empty( $field[ 'sanitize_callback' ] ) ){
						$field[ 'sanitize_callback' ] = 'photoway_sanitize_dropdown_categories';
					}
		...
		*/
		protected function add_panels(){
			...
			foreach ( $this->panels as $id => $panel ) {
				...
				$args = array(
				    'title' => empty( $panel[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'photoway' ) : $panel[ 'title' ],
				);
		...
		*/
		protected function add_sections(){
			...
			foreach ( $this->sections as $id => $section ){
				...
				$args = array(
					'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'photoway' ) : $section[ 'title' ],
				);
		...
		protected function add_control( $field ){
			...
			switch( $field[ 'type' ] ){
				...
				case 'radio-image':
					...
					$this->customize->add_control( new Photoway_Customize_Control_Radio_Image(
						$this->customize,
						$control_id,
						$args
					));
				...
				case 'dropdown-categories':
					...
					$this->customize->add_control( new Photoway_Customize_Category_Control( 
						$this->customize, 
						$control_id, 
						$args 
					) );
				...
				case 'dropdown-posts':
					...
					$this->customize->add_control( new Photoway_Customize_Post_Control( 
						$this->customize, 
						$control_id, 
						$args 
					) );
...
* 
* @since  Photoway 1.0.0
* @uses   Photoway_Customizer::get_instance()->get( $id )
* @return string | bool
*/
if( !function_exists( 'photoway_get_option' ) ){

	function photoway_get_option( $id ){

		return Photoway_Customizer::get_instance()->get( $id );
	}

}

/**
* Retrives default value of Photoway_Customizer.
* 
* @since  Photoway 1.0.0
* @uses   Photoway_Customizer::get_instance()->get_defaults()
* @return object
*/
if( !function_exists( 'Photoway_Customizer_get_defaults' ) ){

	function Photoway_Customizer_get_defaults(){

		return Photoway_Customizer::get_instance()->get_defaults();
	}
}

add_action( 'customize_register', array ( Photoway_Customizer::get_instance(), 'register' ) );
